...
const PDFViewer = ({ pdfUrl, isPremium = false, isBlurred = false, onPageChange }: PDFViewerProps) => {
  ...
  // Block keyboard shortcuts for printing, saving, copying, and screenshot
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Block Ctrl+P (Print), Ctrl+S (Save), Ctrl+C (Copy), Ctrl+Shift+S, Ctrl+U (View Source)
      if (
  ...
  // Note: Dev tools detection removed as it caused false positives
  // The keyboard shortcut blocking is sufficient for security

  // Add CSS to hide content when printing
  useEffect(() => {
  ...
  return (
    <div 
      ...
      style={{
        // CSS to make screenshots show black
    ...
    >
      ...
      {screenshotAttempted && (
        <div className="absolute inset-0 bg-secondary z-50 flex items-center justify-center animate-fade-in">
          <div className="text-center p-8">
            <div className="w-20 h-20 rounded-3xl bg-destructive/20 flex items-center justify-center mx-auto mb-6">
            ...
            <p className="text-muted-foreground">
              Screenshots and screen recordings are disabled for this protected content.
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        >
          ...
          <div className="p-8 md:p-12 space-y-6 no-select" style={{ pointerEvents: isBlurred ? 'none' : 'auto' }}>
