    ...
    return (
        <section className="py-24 lg:py-32 relative overflow-hidden backdrop-blur-[2px]">
            ...
            <div className="container mx-auto px-4 lg:px-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {noteCategories.map((category, index) => (
                        ...
                        >
                            ...
                            >
                                <div className="h-full glass-card rounded-[2.5rem] p-3 hover:border-primary/40 transition-all duration-500 hover:-translate-y-2 border-white/40 dark:border-white/5 dark:bg-slate-900/40 shadow-lg">
                                    {/* Image Area */}
                                    <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden bg-muted/30 mb-6">
                                        <ImageWithSkeleton
                                            src={category.image}
                                            alt={category.title}
                                            className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 origin-center grayscale-[0.3] group-hover:grayscale-0 dark:opacity-80"
                                        />

                                        <div className="absolute top-4 right-4 glass-morphism px-4 py-1.5 rounded-full text-[10px] font-black text-primary border-white/60 dark:border-white/5 shadow-md uppercase tracking-widest bg-white/70 dark:bg-slate-900/60">
                                            {category.count}
                                        </div>
                                    ...
                                    <div className="px-5 pb-6">
                                        <div className="flex items-center justify-between mb-3">
                                            <h3 className="font-display text-2xl font-black tracking-tight group-hover:text-primary transition-colors text-foreground">
                                                {category.title}
                                            </h3>
                                            <div className="w-10 h-10 rounded-full bg-foreground flex items-center justify-center text-background opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-x-4 group-hover:translate-x-0 shadow-md shadow-black/10">
                                                <ArrowRight size={20} />
                                            </div>
                                        </div>
                ...
                {/* Bottom Action */}
                <Reveal animation="reveal-fade" delay={0.3}>
                    <div className="mt-20 text-center">
                        <Link to="/study">
                            <Button variant="outline" className="rounded-full px-12 glass-card border-white/40 dark:border-white/5 dark:bg-slate-900/40 hover:bg-foreground hover:text-background hover:border-foreground h-16 text-sm font-black uppercase tracking-[0.2em] transition-all shadow-lg hover:shadow-xl">
                                View All Categories
                            </Button>
                        </Link>
                    </div>
